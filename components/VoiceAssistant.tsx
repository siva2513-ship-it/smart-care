...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ text, onComplete }) => {
  ...
  return (
    <button
      onClick={speak}
      disabled={isPlaying}
      className={`flex items-center gap-3 px-6 py-4 rounded-full transition-all duration-300 shadow-lg ${
        isPlaying 
    ...
    >
      ...
      <span className="text-xl font-bold">
        {isPlaying ? 'Listening to Instruction...' : 'Read Aloud My Schedule'}
      </span>
    </button>
  );
};
